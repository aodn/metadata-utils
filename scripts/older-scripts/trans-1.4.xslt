...
>
  ...
  <xsl:variable name="imosPointOfContactContact">
	<gmd:pointOfContact>
	  <gmd:CI_ResponsibleParty>
		<gmd:organisationName>
		  <gco:CharacterString>Integrated Marine Observing System (IMOS)</gco:CharacterString>
		...
		<gmd:role>
		  <gmd:CI_RoleCode codeList="http://schemas.aodn.org.au/mcp-2.0/schema/resources/Codelist/gmxCodelists.xml#CI_RoleCode" codeListValue="distributor">pointOfContact</gmd:CI_RoleCode>
		</gmd:role>
	  </gmd:CI_ResponsibleParty>
	</gmd:pointOfContact>
	</xsl:variable>
	...
	<xsl:template match="/mcp:MD_Metadata/gmd:identificationInfo/mcp:MD_DataIdentification/gmd:pointOfContact">
		...
		<xsl:choose>
		  <xsl:when test="gmd:CI_ResponsibleParty/gmd:organisationName/gco:CharacterString[matches( lower-case(text()), 'emii|imos' )]">
			<xsl:copy-of select="$imosPointOfContactContact"/>
		  </xsl:when>
		  <xsl:otherwise>
			<xsl:copy-of select="."/>
		  </xsl:otherwise>
